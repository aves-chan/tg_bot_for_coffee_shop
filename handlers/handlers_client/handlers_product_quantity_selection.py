...
from sql_queries.sql_client import db_select_one_sandwich, db_select_one_breakfasts
from state.state_client import Client_state
from keyboard.keyboard_client import *

client_product_quantity_selection_router = Router()


@client_product_quantity_selection_router.callback_query(StateFilter(Client_state.breakfasts_quantity_selection_state))
async def handler_breakfasts_quantity_selection(cd: types.CallbackQuery, state: FSMContext):
    breakfast = db_select_one_breakfasts(cd.data)
    if breakfast[2] == None:
        await cd.message.delete()
        await cd.message.answer(text=f"""<b>{breakfast[1]}</b>
<b>Описание:</b> {breakfast[2]}

<b>Цена:</b> {breakfast[3]}""", parse_mode=ParseMode.HTML, reply_markup=adding_to_cart_meal(breakfast))
...
@client_product_quantity_selection_router.callback_query(StateFilter(Client_state.sandwich_quantity_selection_state))
async def handler_sandwich_quantity_selection(cd: types.CallbackQuery, state: FSMContext):
    sandwich = db_select_one_sandwich(cd.data)
    if sandwich[2] == None:
        await cd.message.delete()
        await cd.message.answer(text=f"""<b>{sandwich[1]}</b>
<b>Описание:</b> {sandwich[3]}

<b>Цена:</b> {sandwich[4]}""", parse_mode=ParseMode.HTML, reply_markup=adding_to_cart_meal(sandwich))
...
<b>Цена:</b> {sandwich[3]}""", parse_mode=ParseMode.HTML, reply_markup=adding_to_cart_meal(sandwich))
        await state.set_state(Client_state.sandwich_quantity_selection2_state)
        await cd.answer()
